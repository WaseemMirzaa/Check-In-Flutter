...
class CustomFirebaseChatPagination extends StatefulWidget {
  const CustomFirebaseChatPagination({
    required this.query,
  ...
  final Query query;
  final Widget Function(BuildContext, List<Chatmodel>, int) itemBuilder;
  ...
  @override
  State<CustomFirebaseChatPagination> createState() => _FirestorePaginationState();
}

/// The state of the [FirestorePagination] widget.
class _FirestorePaginationState extends State<CustomFirebaseChatPagination> {
  ...
  Future<void> _loadDocuments({bool getMore = true}) async {
    ...
    _streamSub = docsQuery.snapshots().listen((QuerySnapshot snapshot) async {
      ...
      List<Chatmodel> pageMessages = snapshot.docs
          .where((doc) {
        final data = doc.data() as Map<String, dynamic>;
        return messageTimeStamp == null ? true : data[ChatField.TIME_STAMP].compareTo(messageTimeStamp) > 0;
      }).map<Chatmodel>((doc) {
        final data = doc.data() as Map<String, dynamic>;

        messageService.updateUnreadCount(widget.docId, widget.uId, widget.mem);

        if (data['isRead'] == null && data['id'] != widget.uId) {
          messageService.readReceipts(widget.docId, doc.id);
        }
        return Chatmodel.fromJson(data, docID: doc.id);
      }).toList();

      messages..clear()..addAll(pageMessages);
  ...
  Future<void> _setLiveListener() async {
    ...
    _liveStreamSub =
        latestDocQuery.snapshots(includeMetadataChanges: true).listen(
            ...
            messages.insert(0, Chatmodel.fromJson(snapshot.docs.first.data() as Map<String, dynamic>));
  ...
  void _init() async {
    messageTimeStamp = await messageService.getDeleteTimeStamp(widget.docId, widget.uId);
    _loadDocuments();
  }
...
class BuildPagination<T> extends StatelessWidget {
  ...
  final Widget Function(BuildContext, List<Chatmodel> chatModel, int) itemBuilder;
      ...
      case ViewType.list:
        return ListView.separated(
          ...
          itemBuilder: (BuildContext context, int index) {
            if (index >= messages.length) return bottomLoader;
            return itemBuilder(context, messages, index);
          },
